...
function StatCard({ label, value, icon, color }) {
  const colors = {
    blue:   { bg: 'bg-white',   text: 'text-blue-600',   icon: 'bg-blue-50' },
    emerald:{ bg: 'bg-white',   text: 'text-emerald-600',icon: 'bg-emerald-50' },
    amber:  { bg: 'bg-white',   text: 'text-amber-600',  icon: 'bg-amber-50' },
    slate:  { bg: 'bg-white',   text: 'text-slate-600',  icon: 'bg-slate-50' },
  }
        ...
        <div className="flex-1 overflow-auto bg-slate-50/30">
          <div className="max-w-(--breakpoint-2xl) mx-auto px-10 py-10">
            <header className="flex items-center justify-between mb-10">
              <div>
                <h1 className="text-3xl font-black text-slate-900 tracking-tight capitalize">{activeTab === 'new' ? 'Report Issue' : activeTab === 'overview' ? 'Overview' : 'My Tickets'}</h1>
                        ...
                        <tbody className="divide-y divide-slate-50">
                          {tickets.map(t => (
                            <tr key={t.id} onClick={() => setSelectedTicket(t)} className="hover:bg-slate-50/50 cursor-pointer group transition-all">
                              ...
                              <td className="px-10 py-6 text-right text-[10px] font-bold text-slate-400">
